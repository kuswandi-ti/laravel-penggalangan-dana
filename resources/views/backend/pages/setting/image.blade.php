<form action="{{ route('backend.setting.update', !empty($setting->id) ? $setting->id : '') }}?tab=image" method="POST"
                ...
                <div class="mb-3 text-center">
                    @if (!empty($setting->path_image_logo))
                        <img class="img-fluid preview-path_image_logo"
                            src="{{ url(env('PATH_IMAGE_STORAGE') . $setting->path_image_logo) }}" width="200">
                    @else
                ...
                <div class="mb-3 text-center">
                    @if (!empty($setting->path_image_business))
                        <img class="img-fluid preview-path_image_business"
                            src="{{ url(env('PATH_IMAGE_STORAGE') . $setting->path_image_business) }}" width="500">
                    @else
